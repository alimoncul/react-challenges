		...
		if (index !== -1) {
			return (
				<div className="d-flex flex-column">
					...
					<span className="row-cols-auto fs-6 text-start">
						Episodes: {episodes}
					</span>
				</div>
			);
		...
		<div className="App">
			<header className="App-header">
				...
				<div className="container d-flex align-items-center justify-content-center">
					<div
						className="d-flex bg-white position-relative rounded-2 flex-wrap"
						style={{
							minWidth: 600,
							maxWidth: 600,
							padding: 2,
					...
					>
						{selected?.map((s, index) => (
							<div
								key={'ml-spans-' + s}
								className="d-flex col-md-auto gap-2 m-1 p-1 rounded-2 justify-content-center"
								onClick={() => removeSelectedHandler(s)}
								style={{
									cursor: 'pointer',
									outline: 'none',
							...
							>
								<span className="fs-6 align-self-center col-md-auto text-dark">
									{s}
								</span>
								<Cross
									className="fs-5 align-self-center col-md-auto"
									style={{ width: 12, height: 12 }}
								/>
							</div>
						))}
						<input
							className="d-flex form-control shadow-none col-md"
						...
						></input>
						<div
							className="position-absolute p-2 d-flex align-items-center justify-content-center gap-1 h-100"
							style={{ width: 60, right: 0, top: 0 }}
						>
							<Cross
								onClick={() => handleReset()}
								style={{
						...
						<ul
							className={`col-12 list-group w-100 ${optionsVisible ? '' : 'd-none'}`}
							style={{
								maxHeight: 320,
								overflow: 'scroll',
								overflowX: 'hidden',
						...
						>
							{options ? (
								...
								<li
									className="list-group-item d-flex align-items-center justify-content-center gap-4"
									key={'ml-li-not-found'}
								>
									<NotFound
